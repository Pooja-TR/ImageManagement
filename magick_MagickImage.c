  ...
  (JNIEnv *env, jobject self, jint filter)
{
    ...
    image = (Image*) getHandle(env, self, "magickImageHandle", NULL);
    if (image == NULL) {
	throwMagickException(env, "No image to set filter");
	return;
    }
 ...
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_magick_MagickImage_setFuzz
  (JNIEnv *env, jobject self, jdouble fuzz)
{
    ...
    image = (Image*) getHandle(env, self, "magickImageHandle", NULL);
    if (image == NULL) {
	throwMagickException(env, "No image to set fuzz");
	return;
    }
 ...
 * Signature: ()I
 */
JNIEXPORT jdouble JNICALL Java_magick_MagickImage_getFuzz
  (JNIEnv *env, jobject self)
{
    Image *image = NULL;
 ...
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_magick_MagickImage_setBlur
  (JNIEnv *env, jobject self, jdouble blur)
{
    Image *image = NULL;

    image = (Image*) getHandle(env, self, "magickImageHandle", NULL);
    if (image == NULL) {
	throwMagickException(env, "No image to set blur");
	return;
    }
 ...
 * Signature: ()I
 */
JNIEXPORT jdouble JNICALL Java_magick_MagickImage_getBlur
  (JNIEnv *env, jobject self)
{
    Image *image = NULL;
...
/*
 * Class:     magick_MagickImage
 * Method:    drawMagickCaption
 * Signature: (Lmagick/FormatMagickCaptionInfo;)V
 */
JNIEXPORT jint JNICALL Java_magick_MagickImage_drawMagickCaption
    (JNIEnv *env, jobject self, jobject drawInfo, jint leading, jstring caption)
{
    Image 			*image = (Image*) getHandle(env, self, "magickImageHandle", NULL);
    DrawInfo 		*dInfo = (DrawInfo*) getHandle(env, drawInfo, "drawInfoHandle", NULL);
    TypeMetric 		tMetric;
    TypeMetric 		fMetric;
	const char 		*cstrCaption = (*env)->GetStringUTFChars(env, caption, 0);
	int				lines;
	AffineMatrix	affine;
	char			**textList;
	int				i;
	char			*geometry = (char *) AcquireQuantumMemory(30, sizeof(*textList));
	char			*oldGeometry;
	int				y = 0;
	
    if (cstrCaption == NULL) {
		throwMagickException(env, "Unable to get caption value");
		return NULL;
    }

    lines = FormatMagickCaption(image, dInfo, cstrCaption, &tMetric)+1;
	textList=StringToList(cstrCaption);
	oldGeometry = dInfo->geometry;

	GetTypeMetrics(image, dInfo, &fMetric);
	dInfo->geometry = geometry;
	for (i=0; textList[i] != (char *) NULL; i++) {
		sprintf(geometry, "+0+%d", y);
		y += fMetric.height+leading;

		strcpy(dInfo->text, textList[i]);
		AnnotateImage(image, dInfo);
	}
	
	dInfo->geometry = oldGeometry;
	
	for (i=0; textList[i] != (char *) NULL; i++)
		textList[i]=DestroyString(textList[i]);
	
	strcpy(dInfo->text, cstrCaption);
	(*env)->ReleaseStringUTFChars(env, caption, cstrCaption);
	textList=(char **) RelinquishMagickMemory(textList);
	RelinquishMagickMemory(geometry);
	
	return (tMetric.height+leading)*lines-leading;
}
 ...
 * Signature: (D)Lmagick/MagickImage;
 */
JNIEXPORT jobject JNICALL Java_magick_MagickImage_distortImage
  (JNIEnv *env, jobject self, jint method, jdoubleArray args, jboolean bestFit)
{
    Image *image = NULL, *distortedImage = NULL;
    jobject newObj;
    ExceptionInfo exception;
	double *argArray;
	size_t number_args;
	...
	number_args = (*env)->GetArrayLength(env, args);
    argArray = (*env)->GetDoubleArrayElements(env, args, NULL);
    GetExceptionInfo(&exception);

    distortedImage = DistortImage(image, method, number_args, argArray, bestFit, &exception);
    (*env)->ReleaseDoubleArrayElements(env, args, argArray, JNI_ABORT);

    if (distortedImage == NULL) {
		throwMagickApiException(env, "Cannot distort image", &exception);
		DestroyExceptionInfo(&exception);
		return NULL;
    }
    DestroyExceptionInfo(&exception);

    newObj = newImageObject(env, distortedImage);
    if (newObj == NULL) {
		DestroyImages(distortedImage);
		throwMagickException(env, "Unable to create new distorted image");
		return NULL;
    }
 ...
 * Signature: (DD)Lmagick/MagickImage;
 */
JNIEXPORT jobject JNICALL Java_magick_MagickImage_motionBlurImage
  (JNIEnv *env, jobject self, jdouble radius, jdouble sigma, jdouble angle)
{
    Image *image = NULL, *blurredImage = NULL;
    jobject newObj;
    ExceptionInfo exception;
    ...
    GetExceptionInfo(&exception);
    blurredImage = MotionBlurImage(image, radius, sigma, angle, &exception);
    if (blurredImage == NULL) {
	throwMagickApiException(env, "Cannot blur image", &exception);
	DestroyExceptionInfo(&exception);
	return NULL;
 ...
 * Signature: (DD)Lmagick/MagickImage;
 */
JNIEXPORT jobject JNICALL Java_magick_MagickImage_shadeImage
  (JNIEnv *env, jobject self, jboolean gray, jdouble azimuth, jdouble elevation)
{
    Image *image = NULL, *shadedImage = NULL;
    jobject newObj;
    ExceptionInfo exception;
    ...
    GetExceptionInfo(&exception);
    shadedImage = ShadeImage(image, gray, azimuth, elevation, &exception);
    if (shadedImage == NULL) {
	throwMagickApiException(env, "Cannot shade image", &exception);
	DestroyExceptionInfo(&exception);
	return NULL;
    }
    DestroyExceptionInfo(&exception);

    newObj = newImageObject(env, shadedImage);
    if (newObj == NULL) {
	DestroyImages(shadedImage);
	throwMagickException(env, "Unable to create new shaded image");
	return NULL;
    }
